...
fn unnecessary_return_none(checker: &mut Checker, stack: &Stack) {
    for (stmt, expr) in &stack.returns {
        ...
        let mut diagnostic = Diagnostic::new(UnnecessaryReturnNone, (*stmt).into());
...
fn implicit_return_value(checker: &mut Checker, stack: &Stack) {
    for (stmt, expr) in &stack.returns {
        ...
        let mut diagnostic = Diagnostic::new(ImplicitReturnValue, (*stmt).into());
        if checker.patch(diagnostic.kind.rule()) {
            diagnostic.amend(Fix::replacement(
    ...
    match &stmt.node {
        StmtKind::If { body, orelse, .. } => {
            ...
            if let Some(last_stmt) = orelse.last() {
                ...
                let mut diagnostic = Diagnostic::new(ImplicitReturn, stmt.into());
                if checker.patch(diagnostic.kind.rule()) {
...
fn unnecessary_assign(checker: &mut Checker, stack: &Stack, expr: &Expr) {
    if let ExprKind::Name { id, .. } = &expr.node {
        ...
        if !stack.refs.contains_key(id.as_str()) {
            checker
                .diagnostics
                .push(Diagnostic::new(UnnecessaryAssign, expr.into()));
            return;
        }
    ...
    for child in body {
        if matches!(child.node, StmtKind::Return { .. }) {
            let diagnostic = Diagnostic::new(
                ...
                elif_else_range(stmt, checker.locator).unwrap_or_else(|| stmt.into()),
            );
