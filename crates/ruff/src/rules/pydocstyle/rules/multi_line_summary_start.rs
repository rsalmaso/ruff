...
pub fn multi_line_summary_start(checker: &mut Checker, docstring: &Docstring) {
    ...
    if TRIPLE_QUOTE_PREFIXES.contains(&first_line) {
        ...
        {
            let mut diagnostic = Diagnostic::new(MultiLineSummaryFirstLine, docstring.expr.into());
        ...
        {
            let mut diagnostic = Diagnostic::new(MultiLineSummarySecondLine, docstring.expr.into());
            if checker.patch(diagnostic.kind.rule()) {
